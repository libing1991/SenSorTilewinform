...
namespace Tasksdone_Manager
{
    public class tasksDone_Manager : IInterface.IInterface 
    {
        string tableStr = "tasksdone";
        string inQuireStr = "";
        ...
        private string Insert(XmlNodeList nodeList)
        {
            ...
            try
            {
                for (int m = 0; m < nodeList.Count; m++)
                {
                    ...
                    //读取ID
                    string ID = Id_Time.getIdTimeNow();
                    dataDict[primaryKey] = ID;
                    //数据检查
